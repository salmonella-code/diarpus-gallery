...
@section('page_title', $gallery->name)

@section('content')
    <a href="{{ route('video.create', $gallery->slug) }}" class="btn btn-success mb-3"><i
                        ...
                        <div class="product-wrap">
                            <video width="400" controls="controls" preload="metadata" class="video-boder-radius">
                                @if(!$item->files->isEmpty())
                                    <source src="{{ asset('field/'.$gallery->slug.'/video/'.$item->files[0]->folder.'/'.$item->files[0]->name) }}" type="video/mp4">
                                @endif
                            </video>                              
                            <div class="card-img-overlay d-flex align-items-center justify-content-center">
                                <div class="d-flex">
                                    <a href="{{ route('video.download', ['field' => $gallery->slug, 'video' => $item->id]) }}"
                                        class="btn btn-outline-info me-1" target="_blank">
                                        <i class="fas fa-download fa-fw"></i>
                                    </a>
                                    <a href="{{ route('video.show', ['field' => $gallery->slug, 'video' => $item->id]) }}"
                                        class="btn btn-outline-primary me-1">
                                        <i class="fas fa-eye fa-fw"></i>
                                    </a>
                                    <a href="{{ route('video.edit', ['field' => $gallery->slug, 'video' => $item->id]) }}"
                                        class="btn btn-outline-warning me-1">
                                        <i class="fas fa-edit fa-fw"></i>
                                    </a>
                                    <a href="#" onclick="return confirm('Apakah anda yakin ingin menghapus video ini ??');">
                                        <form
                                            action="{{ route('video.destroy', ['field' => $gallery->slug, 'video' => $item->id]) }}"
                                            method="post">
                                            @csrf
                                            @method('delete')
