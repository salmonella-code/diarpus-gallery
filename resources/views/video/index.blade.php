...
@section('content')
    <a href="{{ route('video.create', $galleries->slug) }}" class="btn btn-success mb-3"><i
                    ...
                    <div class="card border-secondary shadow">
                        <div class="product-wrap">
                            ...
                            <div class="card-img-overlay d-flex align-items-center justify-content-center">
                                <div class="d-flex">
                                    <a href="{{ route('video.download', ['gallery' => $galleries->slug, 'video' => $item->id]) }}"
                                        class="btn btn-outline-info me-1" target="_blank">
                                        <i class="fas fa-download fa-fw"></i>
                                    </a>
                                    <a href="{{ route('video.show', ['gallery' => $galleries->slug, 'video' => $item->id]) }}"
                                        class="btn btn-outline-primary me-1">
                                        <i class="fas fa-eye fa-fw"></i>
                                    </a>
                                    <a href="{{ route('video.edit', ['gallery' => $galleries->slug, 'video' => $item->id]) }}"
                                        class="btn btn-outline-warning me-1">
                                        <i class="fas fa-edit fa-fw"></i>
                                    </a>
                                    <a href="#" onclick="return confirm('Apakah anda yakin ingin menghapus video ini ??');">
                                        <form
                                            action="{{ route('video.destroy', ['gallery' => $galleries->slug, 'video' => $item->id]) }}"
                                            method="post">
                                            @csrf
                                            @method('delete')
