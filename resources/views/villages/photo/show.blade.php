...
<div class="d-flex mb-3">
    <a href="{{ route('village.photo.index', $photo->activeVillage->slug) }}" class="btn btn-sm btn-success me-3">
        <i class="fas fa-arrow-alt-circle-left fa-fw align-middle"></i> <span class="align-middle">Kembali</span>
    </a>

    <a href="{{ route('village.photo.edit', ['village'=>$gallery, 'photo'=>$photo->id]) }}" class="btn btn-sm btn-warning me-3"><i class="fas fa-edit fa-fw align-middle"></i></a>

    <a href="#" onclick="return confirm('Apakah anda yakin ingin menghapus foto ini ??');">
        <form action="{{ route('village.photo.destroy', ['village' => $gallery, 'photo' => $photo->id]) }}" method="post">
            @csrf
            @method('delete')
            ...
            <div class="carousel-inner">
                ...
                <div class="carousel-item {{$key == 0 ? 'active' : '' }}">
                    <div class="product-wrap-lg">
                        <img src="{{ asset('village/'.$gallery.'/photo/'.$media->folder.'/'.$media->name) }}" class="card-img-top img-fluid" alt="{{ $media->name }}" id="photo">
                    </div>
                </div>
                @endforeach
