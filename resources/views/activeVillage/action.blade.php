<div class="d-flex">
    <a href="{{ url('/active-village/'.$model->id.'/show') }}" class="btn btn-sm btn-success me-2">
        <i class="fas fa-fw fa-eye align-middle" aria-hidden="true"></i>
    </a>
    <a href="{{ url('/active-village/'.$model->id) }}" class="btn btn-sm btn-warning me-2">
    ...
    <a href="#" onclick="return confirm('apakah anda yakin ingin menghapus semua data yang berkaitan dengan desa ini ??');">
        <form action="{{ url('/active-village/'.$model->id) }}" method="post">
            @csrf
            @method('delete')
