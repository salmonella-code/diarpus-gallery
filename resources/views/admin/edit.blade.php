            ...
            <form action="{{ route('admin.update', $admin->id) }}" method="post">
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-select" name="field" id="field" @error('field') is-invalid @enderror>
                        ...
                        <option value="{{ $field->id }}" {{ old('field', $admin->field->isEmpty() ? '' : $admin->field->first()->id) == $field->id ? 'selected' : '' }}>{{ $field->name }}</option>
                        @endforeach
                    </select>
                    @error('field')
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control  @error('group') is-invalid @enderror" id="group" name="group"
                        placeholder="Masukkan golongan" value="{{ old('group', $admin->group) }}">
                    @error('group')
                        <span class="invalid-feedback" role="alert">
                            "{{ $message }}"
                        </span>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control  @error('position') is-invalid @enderror" id="position"
                        name="position" placeholder="Masukkan jabatan" value="{{ old('position',  $admin->position) }}">
                    @error('position')
                        <span class="invalid-feedback" role="alert">
                            "{{ $message }}"
                        </span>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control  @error('contact') is-invalid @enderror" id="contact"
                        name="contact" placeholder="Masukkan no hp" value="{{ old('contact', $admin->contact) }}">
                    @error('contact')
                        <span class="invalid-feedback" role="alert">
                            "{{ $message }}"
                        </span>
